...
static void _regen_section_header(const Elf_Ehdr_Type *pehdr, const char *buffer, size_t len, Elf_Shdr_Type g_shdr[SHDRS])
{
	...
	if (g_shdr[ARMEXIDX].sh_addr != 0) {
		// ARM32: .text ends at .ARM.exidx
		g_shdr[TEXT].sh_size = g_shdr[ARMEXIDX].sh_addr - g_shdr[TEXT].sh_addr;
	} else {
		// ARM64: no .ARM.exidx — extend .text to .fini_array or .dynamic
		Elf_Addr_Type textEnd = g_shdr[FINIARRAY].sh_addr;
		if (textEnd == 0) textEnd = g_shdr[DYNAMIC].sh_addr;
		if (textEnd > g_shdr[TEXT].sh_addr)
			g_shdr[TEXT].sh_size = textEnd - g_shdr[TEXT].sh_addr;
	}
